...
public class CellLayout extends ViewGroup {
    static final String TAG = "Launcher.CellLayout";
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (LauncherLog.DEBUG_MOTION) {
            LauncherLog.d(TAG, "onInterceptTouchEvent: action = " + action + ",mInterceptTouchListener = " 
...
out:            for (int i = x; i < x + spanX - 1 && x < xCount; i++) {
    ...
    /**
     * Request child layout to recreate display list.
     */
    public void requestChildLayout() {
        if (LauncherLog.DEBUG) {
            LauncherLog.d(TAG, "requestChildLayout: mChildren = " + mChildren);
        }
        if (mChildren != null) {
