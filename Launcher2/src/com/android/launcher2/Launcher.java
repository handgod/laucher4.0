...
public final class Launcher extends Activity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!mModel.isAllAppsLoaded()) {
            ...
            if (LauncherLog.DEBUG_TEMP) {
                LauncherLog.d(TAG, "Inflate progress bar when onCreate: parent = " + appsCustomizeContentParent);
            }
        }
    ...
    protected void onResume() {
		...
		View hostView = mWorkspace.getCurrentDropLayout();
		View mtkWidgetView = mWorkspace.searchIMTKWidget(hostView);
		if (mtkWidgetView != null) {
		    ((IMTKWidget)mtkWidgetView).onResumeWhenShown(mWorkspace.getCurrentPage());
            if (LauncherLog.DEBUG_SURFACEWIDGET) {
                LauncherLog.d(TAG_SURFACEWIDGET, "onResumeWhenShown: mtkWidgetView = " + mtkWidgetView);
            }
		}
    ...
    protected void onPause() {
		...
		View mtkWidgetView = mWorkspace.searchIMTKWidget(hostView);
		if (mtkWidgetView != null) {
			((IMTKWidget) mtkWidgetView).onPauseWhenShown(mWorkspace.getCurrentPage());
            if (LauncherLog.DEBUG_SURFACEWIDGET) {
                LauncherLog.d(TAG_SURFACEWIDGET, "onPauseWhenShown: mtkWidgetView = " + mtkWidgetView);
            }
		}
        mPaused = true;
    ...
    */
    ...
    private void completeAddAppWidget(final int appWidgetId, long container, int screen) {
        if (LauncherLog.DEBUG_LAYOUT) {
            LauncherLog.d(TAG, "(Launcher)completeAddAppWidget: appWidgetId = " + appWidgetId
    ...
    protected void onNewIntent(Intent intent) {
        ...
        if (Intent.ACTION_MAIN.equals(intent.getAction())) {
            ...
            if (alreadyOnHome && mState == State.WORKSPACE && !mWorkspace.isTouchActive() &&
                    openFolder == null) {
            	mWorkspace.controlMTKWidget(mWorkspace.getCurrentPage());
...
//        }
    ...
    public void openFolder(FolderIcon folderIcon) {
    	...
    	View mtkWidgetView = mWorkspace.searchIMTKWidget(hostView);
    	if ( mtkWidgetView != null) {
    		((IMTKWidget)mtkWidgetView).startCovered(mWorkspace.getCurrentPage());
            if (LauncherLog.DEBUG_SURFACEWIDGET) {
                LauncherLog.d(TAG_SURFACEWIDGET, "startCovered: mtkWidgetView = " + mtkWidgetView
                        + ",hostView = " + hostView);
            }
    	}
    ...
    void closeFolder(Folder folder) {
    	...
    	View mtkWidgetView = mWorkspace.searchIMTKWidget(hostView);
    	if ( mtkWidgetView != null ) {
    		((IMTKWidget)mtkWidgetView).stopCovered(mWorkspace.getCurrentPage());
            if (LauncherLog.DEBUG_SURFACEWIDGET) {
                LauncherLog.d(TAG_SURFACEWIDGET, "stopCovered: mtkWidgetView = " + mtkWidgetView
                        + ",hostView = " + hostView);
            }
    	}
        ...
        if (allowLongPress && !mDragController.isDragging()) {
            if (itemUnderLongClick == null) {
                ...
                if (!(itemUnderLongClick instanceof Folder)) {
                    View mtkWidgetView = mWorkspace.searchIMTKWidget(longClickCellInfo.cell);
                    if (mtkWidgetView != null) {
                        ((IMTKWidget) mtkWidgetView).startDrag();
                        if (LauncherLog.DEBUG_SURFACEWIDGET) {
                            LauncherLog.d(TAG_SURFACEWIDGET, "startDrag:mtkWidgetView = "
                                    + mtkWidgetView);
                        }
                    }
    ...
    public void finishBindingItems() {
        ...
        if (LauncherLog.DEBUG) {
            LauncherLog.d(TAG, "(Launcher)finishBindingItems.");
        }
    }
    ...
    public void bindAllApplications(final ArrayList<ApplicationInfo> apps) {
        ...
        if (progressBar != null) {
            if (LauncherLog.DEBUG_TEMP) {
                LauncherLog.d(TAG, "bindAllApplications complete, remove progress bar "
                        + progressBar + ", its parent is " + progressBar.getParent());
            }
