...
public class PagedViewWidget extends LinearLayout implements Checkable {
    ...
    protected void onDetachedFromWindow() {
        ...
        if (LauncherLog.DEBUG_LAYOUT) {
            LauncherLog.d(TAG, "(PagedViewWidget)onDetachedFromWindow: this = " + this);
        }
    ...
    public void applyFromResolveInfo(PackageManager pm, ResolveInfo info,
        ...
        if (LauncherLog.DEBUG) {
            LauncherLog.d(TAG, "(PagedViewWidget)applyFromResolveInfo info = " + info);
        }
        mHolographicOutlineHelper = holoOutlineHelper;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (LauncherLog.DEBUG_MOTION) {
            LauncherLog.d(TAG, "(PagedViewWidget)onTouchEvent event = " + event);
        }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (LauncherLog.DEBUG) {
            LauncherLog.d(TAG, "onLayout: child count = " + this.getChildCount() + ",changed = "
                    + changed + ",left = " + left + ",top = " + top + ",right = " + right
                    + ",bottom = " + bottom + ",mNeedLayout = " + mNeedLayout + ",this = " + this
	...
	// Registered listener layout completion
    private final OnGlobalLayoutListener mGlobalLayoutListener = new OnGlobalLayoutListener() {
        public void onGlobalLayout() {
            LauncherLog.d(TAG, "onGlobalLayout: mFirstLayout = " + mFirstLayout + ",mNeedLayout = " + mNeedLayout);
            if (mFirstLayout && mNeedLayout) {
                PagedViewWidget.this.requestLayout();
            }
            mFirstLayout = false;
            mNeedLayout = false;
            PagedViewWidget.this.getViewTreeObserver().removeGlobalOnLayoutListener(mGlobalLayoutListener);
        }
    };
