...
public class LauncherModel extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (Intent.ACTION_PACKAGE_CHANGED.equals(action)
            ...
            Log.d(TAG, "LOCALE_CHANGED: config = " + context.getResources().getConfiguration());
            forceReload();
        ...
        synchronized (mLock) {
            ...
            if (mCallbacks != null && mCallbacks.get() != null) {
                ...
                if (LauncherLog.DEBUG) {
                    LauncherLog.d(TAG, "(LauncherModel)startLoader mLoaderTask = " + mLoaderTask, new Throwable("startLoader"));
                }
    ...
    public void stopLoader() {
        synchronized (mLock) {
            if (mLoaderTask != null) {
            	if (LauncherLog.DEBUG) LauncherLog.d(TAG, "(LauncherModel)stopLoader mLoaderTask = " + mLoaderTask);
    ...
    private class LoaderTask implements Runnable {
        ...
        private void onlyLoadWorkspace() {
        	if (DEBUG_LOADERS) {
        		Log.d(TAG, "(onlyLoadWorkspace mWorkspaceLoaded = " + mWorkspaceLoaded + ", this = " + this);
        	}
        	if (!mWorkspaceLoaded) {
                loadWorkspace();
                synchronized (LoaderTask.this) {
        ...
        public void run() {
            ...
            keep_running: {
                ...
                if (DEBUG_LOADERS) Log.d(TAG, "First: step1: loading workspace, this = " + this);
                onlyLoadWorkspace();
                if (DEBUG_LOADERS) Log.d(TAG, "First: step2: loading all apps, this = " + this);
                onlyLoadAllApps();
                
                //then bind workspace or all apps
                
                if (loadWorkspaceFirst) {
                	if (DEBUG_LOADERS) Log.d(TAG, "Second step:1 bindWorkspace, this = " + this);
                	bindWorkspace();
                } else {
                	if (DEBUG_LOADERS) Log.d(TAG, "Second step:1 special: bindAllApps, this = " + this);
                	onlyBindAllApps();
                }
                ...
                if (loadWorkspaceFirst) {
                	...
                	if (DEBUG_LOADERS) Log.d(TAG, "Second step:2 special: bindWorkspace, this = " + this);
                	bindWorkspace();
                }
        ...
        private void bindWorkspace() {
            ...
            mHandler.post(new Runnable() {
                public void run() {
                    if (DEBUG_LOADERS) {
                        Log.d(TAG, "Going to start binding widgets soon: this = " + this);
                    }
                }
            });
        ...
        private void loadAllAppsByBatch() {
            ...
            while (i < N && !mStopped) {
                ...
                if( !optr.equals("OP01")) {
                	mAllAppsList.removeWifiSettings();
                }
                ...
                /*mHandler.post(new Runnable() {
                    public void run() {
                        ...
                        if (callbacks != null) {
                            ...
                            Log.i(TAG, "not binding apps: no Launcher activity, this = " + this);
                        }
                    }
                });*/
    ...
    public static void resetCollator() {
        ...
        if (LauncherLog.DEBUG) {
            Log.d(TAG, "(LauncherModel)resetCollator sCollator = " + sCollator
    ...
    synchronized void setFlushCache() {
        LauncherLog.d(TAG, "Set flush cache flag for locale change.");
    ...
    synchronized void flushCacheIfNeeded(HashMap<Object, CharSequence> labelCache) {
        if (LauncherLog.DEBUG) {
            LauncherLog.d(TAG, "flushCacheIfNeeded: sForceFlushCache = " + sForceFlushCache
