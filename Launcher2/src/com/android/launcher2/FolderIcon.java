...
public class FolderIcon extends LinearLayout implements FolderListener {
    ...
    private void onDrop(final ShortcutInfo item, View animateView, Rect finalRect,
    	...
    	if(LauncherLog.DEBUG_MOTION) LauncherLog.d(TAG, "(FolderIcon)onDrop: ShortcutInfo = "+item+
    			", View = "+animateView+", Rect = "+finalRect+", scaleRelativeToDragLayer = "+scaleRelativeToDragLayer+
    			", index = "+index+", Runnable = "+postAnimationRunnable);
        item.cellX = -1;
        item.cellY = -1;
    ...
    public void onDrop(DragObject d) {
    	if(LauncherLog.DEBUG) LauncherLog.d(TAG, "(FolderIcon)onDrop: DragObject = "+d);
        ShortcutInfo item;
