...
public class SearchWidgetProvider extends BroadcastReceiver {
	...
	/**
	 * Broadcast intent action for showing the next voice search hint (if voice
	 * search hints are enabled).
	 */
	...
	/**
	 * Broadcast intent action for updating voice search hint display. Voice
	 * search hints will only be displayed with some probability.
	 */
	 ...
	 * and resetting the 'first seen' voice search timestamp, so we continue to
	 * show them for a while.
	 */
	...
	/**
	 * The {@link Search#SOURCE} value used when starting searches from the
	 * search widget.
	 */
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (AppWidgetManager.ACTION_APPWIDGET_ENABLED.equals(action)) {
		...
		} else if (ACTION_SEARCH_ENGINE_CHANGED.equals(action)) {
			SharedPreferences p = context.getSharedPreferences(
					SearchSettingsImpl.PREFERENCES_NAME, Context.MODE_PRIVATE);
			QsbApplication.get(context).updateSearchEngineExternal(
					p,
					intent.getExtras().getString(
							SearchEngineItemsController.SEARCH_ENGINE_PREF));
			updateSearchWidgets(context);
			...
			QsbApplication.get(context).getSettings()
					.resetVoiceSearchHintFirstSeenTime();
			...
			if (DBG)
				Log.d(TAG, "Unhandled intent action=" + action);
		}
	}
	...
	private static boolean shouldShowVoiceSearchHints(Context context) {
		return (getConfig(context).allowVoiceSearchHints() && !getSettings(
				context).haveVoiceSearchHintsExpired(
				getVoiceSearch(context).getVersion()));
	}

	private static SearchWidgetState[] getSearchWidgetStates(Context context,
			boolean enableVoiceSearchHints) {

		AppWidgetManager appWidgetManager = AppWidgetManager
				.getInstance(context);
		int[] appWidgetIds = appWidgetManager
				.getAppWidgetIds(myComponentName(context));
	...
	private static void considerShowingVoiceSearchHints(Context context) {
		if (DBG)
			Log.d(TAG, "considerShowingVoiceSearchHints");
	...
	private static void showVoiceSearchHintNow(Context context) {
		if (DBG)
			Log.d(TAG, "showVoiceSearchHintNow");
		SearchWidgetState[] states = getSearchWidgetStates(context, true);
		for (SearchWidgetState state : states) {
	...
	private static void voiceSearchHintReceived(Context context,
			CharSequence hint) {
		if (DBG)
			Log.d(TAG, "voiceSearchHintReceived('" + hint + "')");
	...
	private static void scheduleVoiceHintUpdates(Context context) {
		if (DBG)
			Log.d(TAG, "scheduleVoiceHintUpdates");
		if (!shouldShowVoiceSearchHints(context))
			return;
	...
	public static void updateSearchWidgets(Context context) {
		...
		if (DBG)
			Log.d(TAG, "Need voice search hints=" + needVoiceSearchHint);
	...
	private static Intent createQsbActivityIntent(Context context,
			String action, Bundle widgetAppData, Corpus corpus) {
	...
	private static SearchWidgetState getSearchWidgetState(Context context,
		...
		String corpusName = SearchWidgetConfigActivity.getWidgetCorpusName(
				context, appWidgetId);
		Corpus corpus = corpusName == null ? null : getCorpora(context)
				.getCorpus(corpusName);
		if (DBG)
			Log.d(TAG, "Creating appwidget state " + appWidgetId + ", corpus="
					+ corpus);
		...
		if (corpus == null) {
			state.setQueryTextViewHint(context
					.getString(R.string.corpus_label_global));
		} else if (corpus.isWebCorpus()) {
			state.setQueryTextViewHint(QsbApplication.get(context)
					.getSearchEngineInfo().getLabel());
		} else {
			state.setQueryTextViewHint(corpus.getHint());
		}
		state
				.setQueryTextViewBackgroundResource(R.drawable.textfield_search_empty);

		// Text field click
		Intent qsbIntent = createQsbActivityIntent(context,
				SearchManager.INTENT_ACTION_GLOBAL_SEARCH, widgetAppData,
				corpus);
		state.setQueryTextViewIntent(qsbIntent);

		// Voice search button
		Intent voiceSearchIntent = getVoiceSearchIntent(context, corpus,
				widgetAppData);
		state.setVoiceSearchIntent(voiceSearchIntent);
		if (enableVoiceSearchHints
				&& voiceSearchIntent != null
				&& RecognizerIntent.ACTION_WEB_SEARCH.equals(voiceSearchIntent
						.getAction())) {
			state.setVoiceSearchHintsEnabled(true);

			boolean showingHint = SearchWidgetConfigActivity
					.getWidgetShowingHint(context, appWidgetId);
			if (DBG)
				Log.d(TAG, "Widget " + appWidgetId + " showing hint: "
						+ showingHint);
	...
	private static CharSequence formatVoiceSearchHint(Context context,
			CharSequence hint) {
		if (TextUtils.isEmpty(hint))
			return null;
		SpannableStringBuilder spannedHint = new SpannableStringBuilder(context
				.getString(R.string.voice_search_hint_quotation_start));
		spannedHint.append(hint);
		Object[] items = spannedHint.getSpans(0, spannedHint.length(),
				Object.class);
		for (Object item : items) {
			if (item instanceof Annotation) {
				...
				if (annotation.getKey().equals("action")
					...
					spannedHint.setSpan(new StyleSpan(Typeface.BOLD), start,
							end, 0);
				}
			}
		}
		spannedHint.append(context
				.getString(R.string.voice_search_hint_quotation_end));
		return spannedHint;
	}

	private static void rescheduleAction(Context context, boolean reschedule,
			String action, long period) {
		...
		if (reschedule) {
			if (DBG)
				Log.d(TAG, "Scheduling action " + action + " after period "
						+ period);
			alarmManager.setInexactRepeating(AlarmManager.ELAPSED_REALTIME,
					SystemClock.elapsedRealtime() + period, period, intent);
		} else {
			if (DBG)
				Log.d(TAG, "Cancelled action " + action);
		}
	}

	public static void scheduleVoiceSearchHintUpdates(Context context,
			boolean enabled) {
		rescheduleAction(context, enabled, ACTION_CONSIDER_VOICE_SEARCH_HINT,
				getConfig(context).getVoiceSearchHintUpdatePeriod());
	}

	private static void scheduleNextVoiceSearchHint(Context context,
			boolean needUpdates) {
	...
	private static void getHintsFromVoiceSearch(Context context) {
		...
		intent.putExtra(Recognition.EXTRA_HINT_CONTEXT,
				Recognition.HINT_CONTEXT_LAUNCHER);
		if (DBG)
			Log.d(TAG, "Broadcasting " + intent);
		context.sendOrderedBroadcast(intent, null, new HintReceiver(), null,
	...
	/**
	 * Gets the next formatted hint, if there are any hints. Must be called on
	 * the application main thread.
	 * 
	 * @return A hint, or {@code null} if no hints are available.
	 */
	private static CharSequence getNextHint(Context context,
			ArrayList<CharSequence> hints) {
	...
	private static class SearchWidgetState {
		...
		public void setQueryTextViewBackgroundResource(
				int queryTextViewBackgroundResource) {
		...
		private boolean chooseToShowHint(Context context) {
			// this is called every getConfig().getVoiceSearchHintUpdatePeriod()
			// milliseconds
			// we want to return true every
			// getConfig().getVoiceSearchHintShowPeriod() milliseconds
			// so:
			...
			// if p > 1 we won't return true as often as we should (we can't
			// return more times than
			// we're called!) but we will always return true.
			boolean r = (f < p);
			if (DBG)
				Log.d(TAG, "chooseToShowHint p=" + p + "; f=" + f + "; r=" + r);
			return r;
		}
		...
		private void scheduleHintHiding(Context context) {
			AlarmManager alarmManager = (AlarmManager) context
					.getSystemService(Context.ALARM_SERVICE);
			PendingIntent hideHint = createBroadcast(context,
					ACTION_HIDE_VOICE_SEARCH_HINT);
		...
		public void updateShowingHint(Context context) {
			SearchWidgetConfigActivity.setWidgetShowingHint(context,
					mAppWidgetId, mShowHint);
		}

		public boolean considerShowingHint(Context context) {
			if (!mVoiceSearchHintsEnabled || mShowHint)
				return false;
			if (!chooseToShowHint(context))
				return false;
			showHintNow(context);
			return true;
		}
		...
		public void updateWidget(Context context, AppWidgetManager appWidgetMgr) {
			if (DBG)
				Log.d(TAG, "Updating appwidget " + mAppWidgetId);
			RemoteViews views = new RemoteViews(context.getPackageName(),
					R.layout.search_widget);
			// Corpus indicator
			// Before Froyo, android.resource URI could not be used in
			// ImageViews.
			...
			// Query TextView
			views.setCharSequence(R.id.search_widget_text, "setHint",
					mQueryTextViewHint);
			setBackgroundResource(views, R.id.search_widget_text,
			...
			if (mVoiceSearchIntent != null) {
				setOnClickActivityIntent(context, views,
						R.id.search_widget_voice_btn, mVoiceSearchIntent);
				views.setViewVisibility(R.id.search_widget_voice_btn,
						View.VISIBLE);
			} else {
				views
			...
			if (mShowHint && !TextUtils.isEmpty(mVoiceSearchHint)) {
				views.setTextViewText(R.id.voice_search_hint_text,
						mVoiceSearchHint);

				Intent voiceSearchHelp = getVoiceSearchHelpIntent(context);
				if (voiceSearchHelp == null)
					voiceSearchHelp = mVoiceSearchIntent;
				setOnClickActivityIntent(context, views,
						R.id.voice_search_hint, voiceSearchHelp);

				PendingIntent hideIntent = createBroadcast(context,
						ACTION_HIDE_VOICE_SEARCH_HINT);
				views.setOnClickPendingIntent(
						R.id.voice_search_hint_close_button, hideIntent);

				views.setViewVisibility(R.id.voice_search_hint_container,
						View.VISIBLE);
		...
		private void setBackgroundResource(RemoteViews views, int viewId,
				int bgResource) {
			// setBackgroundResource did not have @RemotableViewMethod before
			// Froyo
			if (QsbApplication.isFroyoOrLater()) {
		...
		private void setOnClickActivityIntent(Context context,
				RemoteViews views, int viewId, Intent intent) {
